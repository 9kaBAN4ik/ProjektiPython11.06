...
def save_to_csv(score, user_answers, correct_answers):
    # Salvesta tulemused CSV-failina
    with open('results.csv', mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(['Küsimus', 'Sinu vastus', 'Õige vastus'])
        for i, (user_answer, correct_answer) in enumerate(zip(user_answers, correct_answers)):
            writer.writerow([f'Küsimus {i+1}', user_answer, correct_answer])
    messagebox.showinfo("Edu", "Tulemused on edukalt salvestatud results.csv faili")
...
def display_csv(user_answers, correct_answers):
    ...
    tree.heading("#0", text="Küsimus", anchor=tk.W)
...
def show_score(score, total_questions, user_answers, correct_answers):
    ...
    btn_exit = tk.Button(root, text="Välju", command=root.quit)
